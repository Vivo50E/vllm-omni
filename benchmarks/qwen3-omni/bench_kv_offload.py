"""Benchmark comparing KV cache offloading on vs off for Qwen3-Omni.

Measures TTFP (Time-to-First-Packet), E2E latency, and RTF across
concurrency levels for both KV offload modes. Saves results as JSON.
...
def pcm_bytes_to_duration(num_bytes: int, sample_rate: int = 24000, sample_width: int = 2) -> float:
    """Convert PCM audio bytes to duration in seconds."""
...
async def send_tts_request(
    ...
    """Send a TTS request via chat completions and measure timing metrics."""
    import base64
    ...
    try:
        if stream:
            ...
            async with session.post(api_url, json=payload) as response:
                ...
                result.audio_duration = pcm_bytes_to_duration(len(audio_data))
            ...
            async with session.post(api_url, json=payload) as response:
                ...
                result.e2e = time.perf_counter() - st
                result.ttfp = result.e2e  # For non-streaming, TTFP = E2E
                ...
                for choice in data.get('choices', []):
                    ...
                    if audio and audio.get('data'):
                        ...
                        result.audio_duration = pcm_bytes_to_duration(len(audio_bytes))
                        if result.audio_duration > 0:
                            result.rtf = result.e2e / result.audio_duration
                        break
...
async def run_benchmark(
    ...
    if num_warmups > 0:
        ...
        warmup_tasks = [
            send_tts_request(session, api_url, PROMPTS[i % len(PROMPTS)], voice, language, stream, model=model)
    ...
    async def limited_request(prompt):
        async with semaphore:
            return await send_tts_request(session, api_url, prompt, voice, language, stream, pbar, model=model)
...
def parse_args():
    parser = argparse.ArgumentParser(description="Qwen3-Omni KV offload benchmark client")
    parser.add_argument("--host", type=str, default="127.0.0.1")
    parser.add_argument("--port", type=int, default=8000)
    parser.add_argument("--num-prompts", type=int, default=50)
    parser.add_argument("--max-concurrency", type=int, nargs="+", default=[1, 4, 10])
    ...
    parser.add_argument("--config-name", type=str, default="kv_offload_on")
